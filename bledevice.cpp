...
void BLEDevice::serviceStateChanged(QLowEnergyService::ServiceState s)
{
    ...
    case QLowEnergyService::RemoteServiceDiscovered:
    {
        ...
        const QLowEnergyCharacteristic  uartChar = service->characteristic(QBluetoothUuid(QUuid(UARTUUID)));
        if (!uartChar.isValid()) {
            qDebug() << "Sensor characteristic not found";
            break;
        }
        // UART notify enabled
        const QLowEnergyDescriptor m_notificationDescUART = uartChar.descriptor(QBluetoothUuid::DescriptorType::ClientCharacteristicConfiguration);
...
{
    if (c.uuid() == QBluetoothUuid(QUuid(UARTUUID))) {
        emit newData(v);
    }
}
